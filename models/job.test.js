...
describe("require tech", function () {
    test("works", async function () {
        const requirement = await Job.requireTech(testJobIds[0], testTechIds[2]);
    ...
    test("not found if no such tech", async function () {
        try {
            await Job.requireTech(testJobIds[1], 0);
            fail();
        } catch (err) {
            expect(err instanceof NotFoundError).toBeTruthy();
        }
    ...
    test("not found if no such job", async function () {
        try {
            await Job.requireTech(0, testTechIds[1]);
            fail();
        } catch (err) {
            expect(err instanceof NotFoundError).toBeTruthy();
        }
